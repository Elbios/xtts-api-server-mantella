...
# Configuration
SERVER_URL = "http://127.0.0.1:8020"  # Update to your server URL if needed
...
def test_create_latents():
    ...
    speaker_name = sys.argv[2] if len(sys.argv) > 2 else "test_speaker"  # Still optional for local saving
    language = sys.argv[3] if len(sys.argv) > 3 else "en"  # Still optional for local saving
    ...
    # File upload (no data dict, since server doesn't expect it)
    files = {
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Save latents locally as JSON (using optional speaker_name/language for filename)
